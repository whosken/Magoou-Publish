...
def run(users,contents):
	try:
		def startThread(request,storages):
...
def runEdit(user,users,contents):
	entries = contents.getEntriesWithKeywords(user['preferences'].keys(),startDatetime=user['datetime'])
	if not len(entries) > 0:
		logMessage(__name__,'No contents for '+user['username'])
		return {}
	
	release = _selectCandidates(prefer,entries)
	issue = {
				'username':user['username'],
				'entries':release,
				'public':user['publish'],
			}
	_, data = users.putIssue(issue)
	return data
